...
export default function NewDestinationNumberPage() {
  ...
  const { data: ewallets } = useFetchData<EWallets[]>(
    `/vendor/ewallets`,
    user?.token
  );

  // get specific ewallet id based on slug
  const ewalletId = ewallets?.filter(
    (item: { name: string }) => item.name.toLowerCase() == slug?.toLowerCase()
  )[0].id;
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: 'Nominal Tidak Boleh Kosong' },
              {
                type: 'number',
                min: 10000,
                message: 'Minimum transfer adalah 10.000, mohon isikan kembali',
              },
              {
                type: 'number',
