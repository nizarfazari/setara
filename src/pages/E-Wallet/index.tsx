                    ...
                    {data?.map((item: EWallets) => (
                        ...
                        >
                            <div
                                ...
                                className={`flex justify-center shadow-inner mb-3 ${selectedWallet === item.id ? 'bg-primary-100' : 'bg-primary-300'} rounded-2xl sm:h-[120px] h-[75px] sm:w-[120px] w-[75px] mx-auto items-center cursor-pointer`}>
                                <img src={item.image_path} alt={item.name} className='sm:w-[60px] sm:h-[60px] w-[40px] h-[40px]' />
                            </div>
                            <h1 className='sm:text-heading-6 sm:bg-black-400 sm:font-bold font-semibold text-[12px] leading-[16px]'>{item.name}</h1>
                        </Link>
                    ))}
                    {isLoading && (
                        [1, 2, 3, 4, 5].map((item) => (
