...
const FormTopUp: React.FC<PropsFormTopup> = ({ pathUrl, isTfBa = false }) => {
  ...
  return (
    ...
    >
        ...
        rules={[
          {
            validator(_, value) {
              ...
              if (isTfBa ? value < 1 : value < 10000) {
                return Promise.reject(new Error(isTfBa ? "Minimum transfer adalah 1, mohon isikan kembali" : "Minimum transfer adalah 10.000, mohon isikan kembali"));
              }
      ...
      <Button
        tabIndex={0} className="bg-primary-100 text-white w-full h-10 rounded-xl font-semibold text-body-small md:text-heading-6 md:h-[60px]"
        htmlType="submit"
      >
        Lanjutkan
      </Button>
